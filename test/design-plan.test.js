...
import { validateDesign, compileDesign, compileExploration, validateExploration, applySubjectRequirements, planDesign, explicitBrandName } from '../lib/design-plan.js';
import {planningIssue} from '../lib/planning-error.js';
...
test('truncated output is identified and repaired with a larger bounded budget',async t=>{
  ...
  t.mock.method(globalThis,'fetch',async(url,options)=>{
    if(url.endsWith('/api/tags'))return Response.json({models:[{name:process.env.OLLAMA_PLANNER || 'qwen3.8:latest',capabilities:['completion']}]});
